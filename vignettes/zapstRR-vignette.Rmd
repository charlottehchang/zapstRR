...
Contact: <chc2@alumni.princeton.edu>
...
* `huntRRdata`: RRT data on barbet (**Psilopogon** spp.), bulbul (**Pycnonotus**, **Hypsipetes**, **Iole**, and **Ixos** spp.), and partridge (**Arborophila** spp.) hunting with respondent age (standardized to z-scores) as a covariate.
* `RRdata`: illicit or sensitive behaviors among university undergraduates (e.g. bullying, drug usage).

For more information, please see [@Chang2017RRT].

## Univariate analysis & bootstrapping

`RRunivariate` can be used to evaluate the prevalence or the factors associated with a specific sensitive behavior. For instance, with the `huntRRdata`, one could assess what proportion of adult men hunted barbets in Southwest China. One could also determine whether or not age affected barbet hunting.
...
###==================================================================================
## Univariate prevalence analysis: Barbet hunting
###==================================================================================

RRunivariate(y=huntRRdata['Barbet'], p00=p00, p11=p11)$estimates
```

We see that about 1.2\% of the surveyed adult men in Southwest China hunted barbets, with a standard error of 4.1\%. The 95\% confidence interval for the proportion of individuals who hunted barbets was $[0, 9.3]\%$. 

Below, we evaluate whether or not age predicts barbet hunting:

```{r barbet_glm}
###==================================================================================
## Univariate logistic regression: Barbet hunting
###==================================================================================
RRunivariate(y=huntRRdata[,'Barbet'], x=huntRRdata['Age'], p00=p00, p11=p11)$estimates
```

We see that there is inadequate power to properly estimate the effect of age on the probability of hunting barbets. By design, the package will return `NaN` when the information matrix is singular. In this situation, we can use `RRbootstrap` to generate non-parametric bootstrapped confidence intervals.

```{r barbet_glm_boot}
###==================================================================================
## Univariate logistic regression bootstrap: Barbet hunting
###==================================================================================
RRbootstrap(model='univariate',y=huntRRdata[,1], x=huntRRdata['Age'], p00=p00, p11=p11, reps=50)
```

The bootstrap call indicates that $\beta_{age}$ has a mean of 56.1, and 95\% confidence limits of $[-25.7, 174.8]$. Note that the number of bootstrap replicates is low (`reps=50`); we generally recommend 500 or more to generate more robust estimates.
...
Contrast this $\beta_{age}$ against the one generated above for the univariate barbet logistic regression, which failed to even return a standard error: `r RRunivariate(y=huntRRdata['Barbet'], x=huntRRdata['Age'], p00=p00, p11=p11)$estimates[2,]`.

### Evasive responses

In certain locales, despite implementing RRT, respondents may still feel uneasy answering `yes`, as they may fear reputational or legal repercussions. This phenomenon has been termed **evasive response bias** and the IRT model can correct prevalence and regression analyses by controlling for the rate of one-way lying (refusing to answer `yes`: `noncompliance=TRUE`). Note that this analysis is not possible with `RRunivariate` or `RRsumscore`. `RRbootstrap` can return bootstrap estimates of the IRT model with evasive response bias.
